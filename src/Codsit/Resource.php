...
class Resource {
    ...
    public function __construct($codsit){
        $this->codsit = $codsit;
    }
    ...
    public function request($method,$url,$api_key,$data = NULL){
        try{
            ...
            if($method == "GET") {
                ...
                $response = \Requests::get(Codsit::BASE_URL . $url . $url_params, $headers, $options);

            } else if($method == "POST") {

                $response = \Requests::post(Codsit::BASE_URL . $url, $headers, json_encode($data), $options);

            } else if($method == "PATCH") {

                $response = \Requests::patch(Codsit::BASE_URL . $url, $headers, json_encode($data), $options);

            } else if($method == "DELETE") {

                $response = \Requests::delete(Codsit::BASE_URL. $url . $url_params, $headers, $options);
